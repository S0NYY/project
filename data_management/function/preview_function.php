...
function fileUpload($conn, $fileRef, $field_index, $item_last_id, $item_type_id) {

  if(!empty($fileRef['name'][$field_index])) {
 
    $target_dir = TEST_PHOTO_DIR ;
    $target_file = $target_dir . basename($fileRef["name"][$field_index]);
    $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

      $uploadedFileName = $fileRef['tmp_name'][$field_index];
      $fileName = $fileRef['name'][$field_index];
    ...
    if(isset($_POST["submit"])) {
      $check = getimagesize($fileRef["tmp_name"][$field_index]);
      if($check !== false) {
    ...
    // Check file size
    if ($fileRef["size"][$field_index] > 5000000) {
    ...
    // if everything is ok, try to upload file
      ...
      if (move_uploaded_file($fileRef["tmp_name"][$field_index], $modified_target_file)) {
